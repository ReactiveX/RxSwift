...
extension ObservableRepeatWhenTests {
    func testRepeatWhen_never() {
        ...
        let results = scheduler.start {
            xs.repeatWhen { _ in ys }
        }

        XCTAssertEqual(results.events, [.completed(250)])
        XCTAssertEqual(xs.subscriptions, [Subscription(Defaults.subscribed, 250)])
        XCTAssertEqual(ys.subscriptions, [Subscription(Defaults.subscribed, 201)])
    }
    ...
    func testRepeatWhen_Observable_Never() {
        ...
        XCTAssertEqual(xs.subscriptions, [Subscription(Defaults.subscribed, 250)])
        XCTAssertEqual(ys.subscriptions, [Subscription(Defaults.subscribed, 1000)])
    }
    ...
    func testRepeatObservable_Empty() {
        ...
        XCTAssertEqual(xs.subscriptions, [Subscription(Defaults.subscribed, 450)])
    }
    ...
    func testRepeatWhenObservable_NextError() {
        ...
        let results = scheduler.start {
            xs.repeatWhen { notifications in
                notifications.scan(0, accumulator: { count, _ in
                    if count == 2 {
                        throw testError
                    }
                    return count + 1
        ...
        XCTAssertEqual(xs.subscriptions, [Subscription(Defaults.subscribed, 230)])
    }
    ...
    func testRepeatWhenObservable_Complete() {
        ...
        XCTAssertEqual(xs.subscriptions, [Subscription(Defaults.subscribed, 230)])
        XCTAssertEqual(ys.subscriptions, [Subscription(Defaults.subscribed, 200)])
    }
    ...
    func testRepeatWhenNextComplete() {
        ...
        XCTAssertEqual(xs.subscriptions, [
            Subscription(Defaults.subscribed, 230),
    ...
    func testRepeatWhen3ReleasesResourcesOnError() {
        _ = Observable<Int>.just(1).repeatWhen { e in
            Observable<Int>.error(testError)
        }
        .subscribe()
    }
    #endif
