...
extension ObservableUsingTest {
    ...
    func testUsing_ThrowResourceUsage() {
        ...
        func testUsingReleasesResourcesOnComplete() {
            ...
            _ = Observable<Int>.using({ /* _ in // ⚠️ */ compositeDisposable} , observableFactory: { _ in Observable<Int>.just(1) }).subscribe()
        }

        func testUsingReleasesResourcesOnError() {
            let compositeDisposable = CompositeDisposable(disposables: [])
            _ = Observable<Int>.using({ /* _ in // ⚠️ */ compositeDisposable} , observableFactory: { _ in Observable<Int>.error(testError) }).subscribe()
        }
    #endif
}
