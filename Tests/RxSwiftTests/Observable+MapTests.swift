...
extension ObservableMapTest {
    ...
    func testMap1_DisposeOnCompleted() {
        ...
        func testMapWithIndexReleasesResourcesOnComplete() {
            _ = Observable<Int>.just(1).mapWithIndex { _, _ in true }.subscribe() /* ⚠️ */
        }

        func testMapWithIndex1ReleasesResourcesOnError() {
            _ = Observable<Int>.error(testError).mapWithIndex { _, _ in true }.subscribe() /* ⚠️ */
        }

        func testMapWithIndex2ReleasesResourcesOnError() {
            _ = Observable<Int>.just(1).mapWithIndex { _, _ -> Bool in throw testError }.subscribe() /* ⚠️ */
        }
    #endif
}
