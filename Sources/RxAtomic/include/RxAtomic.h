...
#define Atomic(swift_type, llvm_type) \
    ...
    static __inline__ __attribute__((__always_inline__)) __attribute__((no_sanitize("thread"))) SWIFT_NAME(Atomic##swift_type.initialize(self:_:)) \
    void Atomic##swift_type##_Initialize(Atomic##swift_type * _Nonnull self, llvm_type value) { \
        atomic_init(&self->atom, value);\
    }\
    \
    static __inline__ __attribute__((__always_inline__)) __attribute__((no_sanitize("thread"))) SWIFT_NAME(Atomic##swift_type.load(self:)) \
    llvm_type Atomic##swift_type##_Load(Atomic##swift_type * _Nonnull self) { \
        return atomic_load(&self->atom);\
    }\
    \
    static __inline__ __attribute__((__always_inline__)) __attribute__((no_sanitize("thread"))) SWIFT_NAME(Atomic##swift_type.fetchOr(self:_:)) \
    llvm_type Atomic##swift_type##_FetchOr(Atomic##swift_type * _Nonnull self, llvm_type mask) { \
        return atomic_fetch_or(&self->atom, mask);\
    }\
    \
    static __inline__ __attribute__((__always_inline__)) __attribute__((no_sanitize("thread"))) SWIFT_NAME(Atomic##swift_type.add(self:_:)) \
    llvm_type Atomic##swift_type##_Add(Atomic##swift_type * _Nonnull self, llvm_type value) { \
        return atomic_fetch_add(&self->atom, value);\
    }\
    \
    static __inline__ __attribute__((__always_inline__)) __attribute__((no_sanitize("thread"))) SWIFT_NAME(Atomic##swift_type.sub(self:_:)) \
